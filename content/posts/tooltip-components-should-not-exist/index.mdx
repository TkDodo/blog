...
<img src="./nope.jpg" alt="Nope Hand Lettering On Wood and Glass" />

<Attribution
  name="Daniel Herron"
  url="https://unsplash.com/@herrond"
/>
...
So let's did into the potential problems you can get when using tooltips the wrong way before I'm trying to outline what an alternative world could look like.
...
If you can stick a Tooltip on <Emph>any</Emph> element, someone will. I've seen tooltips in the weirdest of places, like on a certain text in the middle of a sentence - without any indication that there is additional information behind that.
...
- Interactive components like `<Button>` or `<Link>` get an optional `title` prop.<br/>
  This allows us to show additional information if we want to. Interactive elements are usually easily discoverable in the UI, and they will be found with keyboard tabbing.
...
- Expose an additional `<InfoIcon>` component to render an info icon + a tooltip.<br/>
