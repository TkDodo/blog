...
<Attribution name="James Sutton" url="https://unsplash.com/@jamessutton_photography" />
...
One thing that I always found hard to get right is solving conflicts in generated files, like package-lock.json.
...
### Why you should not delete package-lock.json
...
This means no major changes, but minors and patches. I believe this is bad for two reasons:
...
2. This _also_ applies to transitive dependencies. So even if you are using strict versions (without the `^`), the libraries you are using do not.
...
Will your app be fine? Maybe, no one can tell. For me, this would usually mean: "regression test everything".
