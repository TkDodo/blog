...
Forms are an important part in many web applications as the primary means to update data. We are using TanStack Query not only to fetch data ([queries](react-query-as-a-state-manager)), but also to modify it ([mutations](mastering-mutations-in-react-query)), so we need to somehow integrate our beloved async state manager with forms.
...
I have gone on the record already about how I am not a fan of copying state from one state manager to another, be it [putting props to state](putting-props-to-use-state) or copying state from [TanStack Query to local state](practical-react-query#keep-server-and-client-state-separate).
...
If you do not redirect to a different page right after the form submission, it might be a good idea to reset the form _after_ the invalidation has completed. As described in [Mastering Mutations](mastering-mutations-in-react-query#some-callbacks-might-not-fire), you'd likely want to do that in the _onSuccess_ callback of _mutate_. This also works best if you keep state seperated, as you only need to reset to _undefined_ in order for the server state to be picked up again:
