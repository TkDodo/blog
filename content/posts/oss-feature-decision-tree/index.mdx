...
<Attribution
  name="Jens Lelie"
  url="https://unsplash.com/@madebyjens"
/>
...
Some things are straightforward to do in user-land. The above example shows that if you really want to sync your query data reliably somewhere else, you're three lines of code away from doing that. Yes, it contains a `useEffect`, and you might feel a bit _dirty_ when writing that code (as you should) but that's okay - at least it's very explicit instead of hidden behind a callback that would literally have to have the same implementation.
