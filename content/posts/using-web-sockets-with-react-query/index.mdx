...
<RqToc id="using-web-sockets-with-react-query" />
...
<Translations
  translations={[
    ...
    {
      language: '简体中文',
      url: 'https://xfsnowind.github.io/zh-cn/blogs/react-query/using-web-sockets-with-react-query/',
    },
  ]}
/>
...
```jsx:title=partial-updates {10-17}
const useReactQuerySubscription = () => {
  const queryClient = useQueryClient()
  React.useEffect(() => {
    const websocket = new WebSocket('wss://echo.websocket.org/')
