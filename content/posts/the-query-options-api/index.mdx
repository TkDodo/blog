...
Turns out, if you can pass the same object everywhere, it makes a lot of sense to abstract that object away into a query definition. Once you have that, you can pass it everywhere:
...
Which is cool, because it catches typos like the one above. But what if you abstract the whole object away int a constant, like our pattern suggests?
...
TypeScript is quite relaxed in these situations, because at runtime, the "extra" property `stallTime` doesn't hurt. And since `staleTime` is optional, we are now just _not_ passing it. Of course, this is "valid", but it's not what we'd expect, and it can be a costly mistake to find.
...
To be clear, this is isn't at all safer than just using type assertions, but at least `undefined` will be added to the union for us. If we refactor what our `fetchTodos` endpoint returns, we won't be notified here of the new type.
...
However, if we <Emph>combine</Emph> the two patterns, we're getting the best of both worlds: Type-safety, co-location and organization. 🚀
...
const todoQueries = {
  ...
  list: (filters: string) =>
    queryOptions({
      queryKey: [...todoQueries.allLists(), filters],
