...
>
  <p>
    ...
    <TwitterBlueText>@TkDodo</TwitterBlueText>'s React Router blog
    post
  </p>
  <p>Brilliant 👏</p>
  <p>
    <TwitterBlueText>{'dev.to/tkdodo/react-q...'}</TwitterBlueText>
  </p>
  <p>
    <TweetImage>
      <ContactDetailQuery />
...
```ts title="Inlined objects" twoslash
// @errors: 2769
...
```ts title="Manual Type Parameter" twoslash
...
```ts title="Tagged QueryKey" twoslash
...
```ts title=dataTagSymbol twoslash
import { QueryClient, queryOptions } from '@tanstack/react-query'
type Todo = { id: number; title: string }
declare function fetchTodos(): Promise<Todo[]>
const queryClient = new QueryClient()
...
```ts title="Custom Hooks" twoslash
...
```ts title="Query Factory" twoslash
import { queryOptions } from '@tanstack/react-query'
type Todo = { id: number; title: string }
declare function fetchTodos(filters?: string): Promise<Todo[]>
declare function fetchTodo(id: number): Promise<Todo>
// ---cut---
const todoQueries = {
  all: () => ['todos'],
