---
title: TanStack Query meets React Router
description: TanStack Query and React Router are a match made in heaven.
date: 2022-08-28
banner: ./rq-rr.jpeg
tags:
  - ReactJs
...
- <b>#16: TanStack Query meets React Router</b>
...
With React Router coming into the data fetching game, it is naturally interesting to understand how this competes or correlates with existing data fetching and caching libraries like TanStack Query. So let me spoil it right here:
...
- Would we still want TanStack Query now that we can fetch in the route?
- If we already use TanStack Query, do we want to (and how can we) leverage the new React Router features?
...
The big drawback of not having a cache shows when you go to Contact 2 and then back to Contact 1 again. If you are used to TanStack Query, you will know that data for Contact 1 is cached already, so we can show it instantly and kick of a background refetch if data is considered stale. With the loader approach, we will have to fetch that data again (and wait for it to finish fetching!), even though we have already fetched it before.

And that is exactly where TanStack Query comes in.

What if we can use the loader to pre-fill the TanStack Query Cache, but still _useQuery_ in the component to get all the TanStack Query goodies like _refetchOnWindowFocus_ and showing stale data instantly? To me, this sounds like the best of both worlds. The router is responsible for fetching data early (if we don't have it), and TanStack Query is responsible for caching and keeping the data fresh.
...
import { useQuery } from '@tanstack/tanstack-query'
import { getContact } from '../contacts'
...
The loader is not a hook, so we can't _useQueryClient_. Importing the QueryClient directly is something that [I'm not recommending](tanstack-query-fa-qs#why-should-i-usequeryclient), so passing it explicitly seems like the best alternative.
...
Thanks to [Matt Pocock](https://twitter.com/mattpocockuk) and his [contribution](https://github.com/TanStack/query/pull/3834) to TanStack Query v4, we can now exclude _undefined_ from the union if _initialData_ is provided.
...
The next piece of the puzzle involves query invalidation. Here is how an action would look without TanStack Query, straight from the tutorial (yes, this is all it takes to perform an update):
...
I'm very excited about the new React Router release. It's a great step forward to enable all applications to trigger fetches as early as possible. However, it is not a replacement for caching - so go ahead and combine React Router with TanStack Query to get the best of both worlds. 🚀

If you want to explore this topic some more, I've implemented the app from the tutorial and added TanStack Query on top of it - you can find it in [the examples of the official docs](https://tanstack.com/query/v4/docs/examples/react/react-router).
