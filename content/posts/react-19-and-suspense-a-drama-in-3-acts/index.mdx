...
This release is packed with goodies, and I was excited to upgrade React Query to see if there were any problems. I was quite busy at the time with work and finishing the [🔮 query.gg](https://query.gg) course, but about a month later, we released v5.39.0, which is compatible with React 19:
...
So after the React 19 upgrade in React Query itself, I continued working on the suspense lesson of the course. We have one example in there were we're showing how to reveal content at the same time, but still have all requests fetch in parallel. As shown in the [react docs](https://react.dev/reference/react/Suspense#revealing-content-together-at-once), we can achieve this by by putting both components as siblings into the same suspense boundary. The example looks roughly like this:
...
Later that evening, I had the chance to talk to [Sathya Gunasekaran](https://x.com/_gsathya), who worked on the react compiler and v19:
