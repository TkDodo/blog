...
Now _everything_ title related is in _useTitle_, encapsulating all the logic with it.

We can take this one step further: If we only intend to show the title in the document title and nowhere else, we can keep the title value in the hook and expose only the setter, producing a minimal interface:
...
Testing the _useTitle_ hook without having to test the component where it's being used has the advantage that you don't have to think about all the other things that are going on in that component, like page tracking. Testing custom hooks is very similar to testing any other util function:
