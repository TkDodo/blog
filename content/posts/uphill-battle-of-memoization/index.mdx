...
To recap: When react renders a component tree, it will do so top-down, for all children. Once a render has started, there isn't really a way for us to stop it. Mostly, that's a good thing, because renders make sure that we see a correct reflection of our state on the screen. Also, renders are usually fast.
...
So using `React.memo` is a bit of a minefield, and choosing one of the proposed alternatives seems way better to me. But sometimes, we seemingly can't avoid memoizing a component. Let's take a look at the example I've seen on twitter that sparked the ide for this blogpost:
...
Remember when I said that once a render started, we have no way of stopping it? That's still tue, but what if we'd stop the render from starting in the first place ... 🤔
