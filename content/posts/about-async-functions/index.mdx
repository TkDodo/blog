...
<Attribution name="WanderLabs" url="https://unsplash.com/@wanderlabs" />
...
<p style="padding-left: 3rem; margin-top: -1rem">— Someone, somewhen</p>
...
I first stumbled upon this when working with [query cancellation in tanstack-query](https://react-query.tanstack.com/guides/query-cancellation). Here, tanstack-query wants us to attach a `.cancel` method on our resulting promise. Surprisingly, this doesn't quite work in async functions:
...
Because we are in an async function, a _new_ promise will be returned at the end of it, even if we already return a promise ourselves! [Here](https://www.carlrippon.com/cancelling-requests-with-tanstack-query/) is a great article if you want to see how query cancellation can work even with async functions.

**Update**

Since [v3.30.0](https://react-query.tanstack.com/guides/query-cancellation), TanStack Query will directly provide you the AbortSignal. There is no need to add a _cancel_ method to the promise, and this way of cancelling queries has been entirely removed in v4.
