...
From time to time, I get bug reports that the `queryFn` isn't called even though it should. When that happens, the most likely reason is the use of `initialData` together with `staleTime`:

```js:title=initialData-and-staleTime
const { data } = useQuery({
  queryKey: ['todos'],
  queryFn: fetchTodos,
  initialData: [],
...
```js:title=placeholderData-and-staleTime
const { data } = useQuery({
  queryKey: ['todos'],
  queryFn: fetchTodos,
  placeholderData: [],
