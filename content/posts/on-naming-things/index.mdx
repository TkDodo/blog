...
Let me try to explain why it's _not_ great with a different example.
...
But maybe, in the future, we will do something else, or something additional, like showing an _Overlay_.
Do we want to read `showLoading && <Overlay/>` in our code?

Additionally, if we only take a look at our JSX (for example, because that condition is imported from somewhere) ,
...
and we can also change what it means to be initializing without having to change the name.
...
It's the same problem. _handleClick_ doesn't tell us anything about what is going to happen,
and it also couples it to a click event of some sort.

If we change the UI from a button to some other component that has an _onChange_ event,
we would now have to re-name our event handler, unless we are fine with `onChange={handleClick}` (probably not)

The two important takeaways are:

-   Would the name change if the implementation changes (a)?
-   Would the name change if the usage of the function changes (b)?

If so, it's probably not a very good name.

<Comments>What are your favorite rules on naming things? Leave a comment below ⬇️</Comments>
