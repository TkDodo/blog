...
description: Any is not among my favorite TypeScript types, but it is the one you will inevitably encounter. Let's see what makes it so dangerous.
date: 2021-12-04
...
[Any](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#any) is not among my favorite TypeScript types, but it is the one you will inevitably encounter, if you like it or not. This might not be a problem in some situations, but can be devastating in others. Let's dive into it:
...
- You're [calling JavaScript from TypeScript](./calling-java-script-from-type-script) - such functions will all just return _any_.
