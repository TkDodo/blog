...
```ts:title=queryFn-transformation {5}
...
```ts:title=render-transformation {12-15}
const fetchTodos = async (): Promise<Todos> => {
  const response = await axios.get('todos')
...
Since React Query has [tracked queries](react-query-render-optimizations#tracked-queries) turned on per default since v4, spreading `...queryInfo` is no longer recommended, because it invokes getters on all properties.
...
```ts:title=select-memoizations {8-9,16-20}
...
🟡 &nbsp; structural sharing is performed twice (I will also talk about this in more detail in [Part 3](react-query-render-optimizations))
