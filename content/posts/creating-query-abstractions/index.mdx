...
description: Building reusable abstractions on top of libraries that rely on type inference can be deceptively difficult, which is why we need better ways to build those abstractions ...
date: 2026-02-23
banner: ./creating.jpg
tags:
  - ReactJs
...
As the tweet shows, we can add more type parameters to our own abstractions, but this is takes us further away from code that looks like <Emph>Just JavaScript™</Emph>. The promise was that those libraries do the ugly TypeScript stuff for us so we don't have to ...
...
And this just works! With all options, full type inference, looks like JavaScript, absolutely straight-forward. You can of course still create custom hooks if you want to, but they should likely be built on top of `queryOptions`, as that's the first abstraction building block you should be reaching for. Simplicity is king, and it doesn't get any simpler than this. 👑
