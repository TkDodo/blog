...
Getting optimistic updates right in TypeScript is not an easy feat, so we've decided to add it as an explicit [example](https://react-query.tanstack.com/examples/optimistic-updates-typescript) to the docs.
...
For the most parts, typing _useInfiniteQuery_ is no different from typing _useQuery_. One noticeable gotcha is that the _pageParam_ value, which is passed to the _queryFn_, is typed as _any_. Could be improved for sure, but as long as it's _any_, it's probably best to explicitly annotate it:
            ...
            queryFn: ({ queryKey: [url] }) => {
                ...
                if (typeof url === 'string') {
                    const { data } = await axios.get(
                        `https://jsonplaceholder.typicode.com${url.toLowerCase()}`
                    )
                    return data
                }
