---
title: TanStack Query and TypeScript
...
date: 2021-05-16
banner: ./tanstack-query-typescript.jpeg
tags:
  - ReactJs
  - TanStack Query
  - TypeScript
...
<img src="./tanstack-query-typescript.jpeg" />
...
- <b>#6: TanStack Query and TypeScript</b>
...
<Translations>
  {[
    ...
    {
      language: 'Español',
      url: 'https://rubenvara.io/tanstack-query/tanstack-query-typescript',
    },
  ]}
</Translations>
...
TanStack Query was initially written in JavaScript (v1), and was then re-written to TypeScript with v2. This means that right now, there is very good support for TypeScript consumers.

There are however a couple of "gotchas" when working with TypeScript due to how dynamic and unopinionated TanStack Query is. Let's go through them one by one to make your experience with it even better.

## Generics

TanStack Query heavily uses [Generics](https://www.typescriptlang.org/docs/handbook/2/generics.html). This is necessary because the library does not actually fetch data for you, and it cannot know what _type_ the data will have that your api returns.
...
let result = identity('tanstack-query')
```

### Partial Type Argument Inference

...doesn't exist in TypeScript yet (see this [open issue](https://github.com/microsoft/TypeScript/issues/26242)). This basically means that if you provide _one_ Generic, you have to provide _all_ of them. But because TanStack Query has default values for Generics, we might not notice right away that they will be taken. The resulting error messages can be quite cryptic. Let's look at an example where this actually backfires:
...
If you (like me) like to keep your api layer separated from your queries, you'll need to add type definitions anyways to avoid _implicit any_, so TanStack Query can infer the rest:
...
This has nothing to do with TanStack Query, it is just how TypeScript works. [@danvdk](https://twitter.com/danvdk) has a good explanation for this behaviour
