...
<RqToc id="effective-react-query-keys" />
...
```js:title=invalidate-all-lists {10-13}
function useUpdateTitle() {
  return useMutation({
    mutationFn: updateTitle,
    onSuccess: (newTodo) => {
