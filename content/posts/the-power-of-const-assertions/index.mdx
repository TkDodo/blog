...
but it can also become quite boilerplaty pretty fast.
...
Of course, this only works because of the const assertions, and if you remove it or forget it, _Variant_ will just be of type number.
...
I'll take that safety any day (and tuck _Extract_ away in a util).
...
The thing is: You _can_ pass a mutable Array or Object into a method that takes a readonly Array or Object, but not the other way around.
The reason is quite simple: if my function accepts an _Array_, the function might sort it, so you cannot pass a ReadonlyArray to it.

By making the parameter readonly, you just guarantee that you are not mutating it (which is always good - don't mess with function parameters)
and you make people's lives easier if they are using const assertions in the process.

If you are a library author, I strongly recommend making all inputs to your functions readonly.

<Comments>Do you also prefer const assertions? Leave a comment below ⬇️</Comments>
