...
It might also be it's the resemblance to type casts that make people afraid of using them.
...
The reason is quite simple: if my function accepts an _Array_, the function might mutate / sort it, so you cannot pass a ReadonlyArray to it.
