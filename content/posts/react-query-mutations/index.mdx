---
title: Mastering Mutations in TanStack Query
description: Learn all about the concept of performing side effects on the server with TanStack Query.
date: 2021-10-09
...
- <b>#12: Mastering Mutations in TanStack Query</b>
...
We've covered a lot of ground already when it comes to the features and concepts TanStack Query provides. Most of them are about _retrieving_ data - via the _useQuery_ hook. There is however a second, integral part to working with data: updating it.

For this use-case, TanStack Query offers the _useMutation_ hook.
...
By that, I mean that queries mostly run automatically. You define the dependencies, but TanStack Query takes care of running the query immediately, and then also performs smart background updates when deemed necessary. That works great for queries because we want to keep what we see on the screen _in sync_ with the actual data on the backend.

For mutations, that wouldn't work well. Imagine a new todo would be created every time you focus your browser window 🤨. So instead of running the mutation instantly, TanStack Query gives you a function that you can invoke whenever you want to make the mutation:
...
Mutations are, per design, not directly coupled to queries. A mutation that likes a blog post has no ties towards the query that fetches that blog post. For that to work, you would need some sort of underlying schema, which TanStack Query doesn't have.

To have a mutation reflect the changes it made on our queries, TanStack Query primarily offers two ways:

### Invalidation

This is conceptually the simplest way to get your screen up-to-date. Remember, with server state, you're only ever displaying a snapshot of data from a given point in time. TanStack Query tries to keep that up-to-date of course, but if you're deliberately changing server state with a mutation, this is a great point in time to tell TanStack Query that some data you have cached is now "invalid". TanStack Query will then go and refetch that data if it's currently in use, and your screen will update automatically for you once the fetch is completed. The only thing you have to tell the library is _which_ queries you want to invalidate:
...
I'm showing some more examples of direct updates and the combination of both approaches in [#8: Effective TanStack Query Keys](effective-tanstack-query-keys#structure).
...
Promises returned from the mutation callbacks are awaited by TanStack Query, and as it so happens, _invalidateQueries_ returns a Promise. If you want your mutation to stay in _loading_ state while your related queries update, you have to return the result of _invalidateQueries_ from the callback:
...
Handling errors is not necessary with _mutate_, because TanStack Query catches (and discards) the error for you internally. It is literally implemented with: *mutateAsync().catch(noop)*😎
...
To read more on why that is currently necessary, have a look at [this discussion](https://github.com/tannerlinsley/tanstack-query/discussions/1226).
