...
We have an existing REST API, don't really experience problems with over-fetching, it just works,
and clearly, there weren't enough pain points to warrant a switch,
given that you have to also adapt your backend, which isn't quite so simple.

Yet I still envied the simplicity of how data fetching can look like on the client,
...
I want to focus more on some practical tips that go beyond the docs that might be useful when you are already working with the library.
...
-   StaleTime: The duration until a query transitions from fresh to stale.
    ...
    but a background refetch can happen [under certain conditions](https://react-query.tanstack.com/docs/guides/caching).
...
I have rarely ever needed to temper with the _cacheTime_.
...
and as soon as the user changes his selection, we would update that local state,
...
[initialData](https://react-query.tanstack.com/docs/guides/initial-query-data#initial-data-from-cache).
...
and they will still see the updated list once the background refetch finishes.
Please note that prior to v3, you would also need to set the _initialStale_ property to actually trigger a refetch.

I think this is a good ux improvement with just a few lines of code.
...
The main reason for that is that you implicitly opt out of all background updates that React Query does for you,
...
I hope that these practical tips will make it even easier for you to get started with React Query, so go check it out :)

<Comments>
    Have you worked with React Query yet? Do you have further questions? Let me know in the
    comments below ⬇️
</Comments>
