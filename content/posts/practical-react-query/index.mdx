...
The important part of that demo is that we never put the value that we get from React Query into local state.
...
-   Turn queries on and off<br />
    We have one query that polls data regularly thanks to _refetchInterval_, but we can turn it off if a Modal is open to avoid
...
<Comments>
    I hope that these practical tips will make it even easier for you to get started with React
    Query, so go check it out :) If you have any further questions, please let me know in the
    comments below ⬇️
</Comments>
