...
-   You can keep all usages of one queryKey (and potentially type definitions) in one file.
-   If you need to tweak some settings or add some data transformation, you can do that in one place.

```ts:title=feature/widget/queries.ts
type Widget = {
    id: string
    title: string
}

const fetchWidget = async (id: number): Promise<Widget> => {
    const response = await axios.get(`widget/${id}`)
    return response.data
}

export const useWidgetQuery = (id: number) => useQuery(['widgets', id], () => fetchWidget(id))
```
