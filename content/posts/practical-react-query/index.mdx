---
title: Practical TanStack Query
description: Let me share with you the experiences I have made lately with TanStack Query. Fetching data in React has never been this delightful...
date: 2020-11-16
banner: ./tanstack-query.png
tags:
  - ReactJs
  - TanStack Query
---
...
<img src="./tanstack-query.png" />

---

- <b>#1: Practical TanStack Query</b>
...
<Translations>
  {[
    ...
    {
      language: 'Português',
      url: 'https://dbrno.vercel.app/pt-BR/blog/practical-tanstack-query',
    },
    {
      language: '日本語',
    ...
    {
      language: 'Español',
      url: 'https://rubenvara.io/tanstack-query/consejos-practicos-tanstack-query/',
    },
    {
      language: '简体中文',
...
Enter [TanStack Query](https://react-query.tanstack.com/).

Made by the open sourcerer [Tanner Linsley](https://github.com/tannerlinsley) in late 2019,
TanStack Query takes the good parts of Apollo and brings them to REST.
...
This article is not going to be an introduction to TanStack Query though.
...
and Tanner has a TanStack Query [Essentials Course](https://learn.tanstack.com/) you can take if you want to get familiar with the library.
...
but also got involved in the TanStack Query community, answering questions on Discord and in GitHub Discussions.

### The Defaults explained

I believe the TanStack Query [Defaults](https://react-query.tanstack.com/guides/important-defaults)
are very well chosen, but they can catch you off guard from time to time, especially at the beginning.

First of all: TanStack Query does _not_ invoke the _queryFn_ on every re-render, even with the default _staleTime_ of zero.
...
and TanStack Query is doing a _refetchOnWindowFocus_, which is a great feature for production:
...
Most of the time, if you want to change one of these settings, it's the _staleTime_ that needs adjusting.
I have rarely ever needed to tamper with the _cacheTime_.
There is a good [explanation by example](https://react-query.tanstack.com/guides/caching#a-detailed-caching-example)
in the docs as well.

### Use the TanStack Query DevTools
...
Because TanStack Query will trigger a refetch whenever the query key changes.
So when we pass a variable parameter to our _queryFn_, we almost always want to fetch data when that value changes.
...
and TanStack Query will automatically trigger the refetch for us, because the query key changes.
...
I don't think I have ever passed a variable to the _queryFn_ that was _not_ part of the _queryKey_, too.
...
Please note that prior to v3, you would also need to set the _initialStale_ property to actually trigger a background fetch.
...
If you get data from _useQuery_, try not to put that data into local state.
The main reason is that you implicitly opt out of all background updates that TanStack Query does for you,
...
So if you do that on purpose, make sure to _not_ fire off unnecessary background refetches by setting _staleTime_:
...
The important part of this demo is that we never put the value that we get from TanStack Query into local state.
...
The _useQuery_ hook has many options that you can pass in to customize its behaviour,
and the _enabled_ option is a very powerful one that _enables_ you to do many cool things (pun intended).
...
- Turn queries on and off<br />
  We have one query that polls data regularly thanks to _refetchInterval_, but we can temporarily pause it if a Modal is open to avoid
...
If you tamper with the queryCache (_queryClient.setQueryData_), it should only be for optimistic updates
...
Even if it's only for wrapping one _useQuery_ call, creating a custom hook usually pays off because:

- You can keep the actual data fetching out of the ui, but co-located with your _useQuery_ call.
