...
I love [these defaults](https://react-query.tanstack.com/guides/important-defaults), but as I said before, they are geared towards keeping things up-to-date, _not_ to minimize the amount of network requests. This is mainly because _staleTime_ defaults to _zero_, which means that evey time you e.g. mount a new component instance, you will get a background refetch. If you do this a lot, especially with mounts in short succession that are not in the same render cycle, you might see _a lot_ of fetches in the network tab. That's because React Query can't deduplicate in such situations:
