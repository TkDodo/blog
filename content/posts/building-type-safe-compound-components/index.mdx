...
This doesn't mean that compound components are _always_ a good fit. Sometimes, using `props` is just better. A common example we'll see regarding compound components is a `Select` with `Options`, likely because that's also how it works in HTML.
