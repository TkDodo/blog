...
bluesky: https://bsky.app/profile/tkdodo.eu/post/3m2zeo2e3gs27
tags:
  - ReactJs
  - TanStack Router
  - TypeScript
...
```tsx title="useParams" {8} twoslash
import {
  createFileRoute,
  createRootRoute,
  createRouter,
...
```tsx title="params parsing" {5-7} twoslash
import {
  createFileRoute,
  createRootRoute,
  createRouter,
...
```tsx title="dashboardId number" {2,3} twoslash
import {
  createFileRoute,
  createRootRoute,
  createRouter,
...
const ParentRoute = createFileRoute('/dashboard/$dashboardId')({
  component: Dashboard,
  params: {
    parse: type({ dashboardId: 'string.integer.parse' }).assert,
  },
...
```tsx title="Debug Search Param" {2} twoslash
...
```tsx title="useSearch" {2,3} twoslash
import {
  createFileRoute,
...
```tsx title="Search Middleware" {3-5} twoslash
import {
  createRootRouteWithContext,
  stripSearchParams,
} from '@tanstack/react-router'
...
type RouteContext = {}
declare const Root: () => ReactElement
// ---cut---
export const Route = createRootRouteWithContext<RouteContext>()({
  validateSearch: type({ debug: 'boolean=false' }).assert,
  search: {
...
```tsx title="Merged Search Params" {4,9,10} twoslash
import {
  createFileRoute,
  createRootRouteWithContext,
  createRouter,
...
```tsx title="Router Context" {5-7} twoslash
import {
  createRootRouteWithContext,
  createRouter,
} from '@tanstack/react-router'
...
const routeTree = createRootRouteWithContext<RouteContext>()()
// ---cut---
...
```tsx title="QueryClient in Loader" {2-3} twoslash
import {
  createFileRoute,
  createRootRouteWithContext,
  createRouter,
...
```tsx title="beforeLoad" {2,4} twoslash
import {
  createFileRoute,
  createRootRouteWithContext,
  createRouter,
...
```tsx title="useRouteContext" {2,3} twoslash
import {
  createFileRoute,
  createRootRouteWithContext,
  createRouter,
