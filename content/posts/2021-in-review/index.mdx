...
>
  ...
  <br />
  <br />
  Open source work has taken up a good part of my daily routine now, which
  can be quite distracting. I will now try to bundle it with a fixed block
  each Friday. This will likely reduce my responsiveness, so please bear
  with me
</Tweet>
...
>
  I'm super proud to announce that my blog has made it as a community
  resource into the official react-query docs 🎊 You can now read an
  excerpt of all my RQ related articles here:{' '}
  <Text color="var(--theme-ui-colors-twitterBlue)">
    https://react-query.tanstack.com/community/tkdo...
  </Text>
  <br />
  <br />
  and the docs also link to them from other places. Thank you <Text color="var(--theme-ui-colors-twitterBlue)">
    @tannerlinsley
  </Text>
</Tweet>
...
>
  Hmm, why are the zustand docs recommending to memoize selectors with
  useCallback
  <Text color="var(--theme-ui-colors-twitterBlue)">@dai_shi</Text> ? Especially
  with the example:
  <br />
  <br />
  ...
  <br />
  <br />
  it really seems like a premature optimization, and ppl jump on it cause
  its in the docs
</Tweet>
...
>
  Okay apparently it’s not obvious to everybody that you DO NOT need to
  copy data from react-query anywhere else (context, redux,…) to access
  it everywhere. I will have to write a blog post about it I guess (and
