...
description: 5 things you need to know about useState
date: 2021-02-13
...
[useState](https://reactjs.org/docs/hooks-reference.html#usestate) is pretty straight forward to use. A value, a setter function, an initial state. What hidden gems could possibly be there to know about? Well, here are 5 things you can profit from on a daily basis that you might not have known:

### 1: The functional updater

_setState_ had it, and _useState_ has it, too: The functional updater. Whenever you calculate a new result depending on the _previousValue_ of your state, you should use the functional updater:
...
Each click will only increment the count once, because both calls to _setCount_ closure over the same value (_count_). It's important to know that _setCount_ will _not_ immediately set the count. The useState updater only schedules an update. It basically tells react: Please set this value to the new value, somewhen. And we are telling React twice: Please set the count to two. React does so, but it is probably not what we intended to say. What we wanted to say is: Please _increment_ the value. The functional updater form ensures this:
...
The functional updater form can also help you to avoid dependencies for _useEffect_, _useMemo_ or _useCallback_. Suppose you want to pass an incrementBy function to a memoized child component. We can make sure the function doesn't change too often with _useCallback_, but if we closure over _count_, we will still create a new reference whenever count changes. The functional updater avoids this problem altogether:

```js:title=avoiding-dependencies
const [count, setCount] = React.useState(0)

// 🚨 will create a new function whenever count changes because we closure over it
const inremenctBy = React.useCallback((value) => setCount(count + value), [count])

// ✅ avoids this problem by not using count at all
const inremenctBy = React.useCallback(
    (value) => setCount((previousCount) => previousCount + value),
    []
)
```
...
The first 3 of those 5 things are actually mentioned directly in the Hooks API Reference of the official React docs I linked to at the very beginning 😉. But if you didn't know about these things before - now you do!
