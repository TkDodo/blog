...
date: 2025-07-28
banner: ./useless.jpg
bluesky: https://bsky.app/profile/tkdodo.eu/post/3luzyua2qqc2b
tags:
  - ReactJs
  - JavaScript
  - useCallback
...
```tsx title="the latest ref" {2-6, 8}
export function useHotkeys(hotkeys: Hotkey[]): {
...
This would make `onKeyDown` _not_ reactive, it would be able to always "see" the latest values of `hotkeys`, and it would be referentially stable between renders. The best of all worlds, without having to write a single useless `useCallback` or `useMemo`.
