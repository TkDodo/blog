...
function App() {
    ...
    return (
        <div>
            ...
            <DetailView initialEmail={email} setEmail={setEmail} />
        </div>
    )
}
...
We want to reset the state whenever another person is selected.
...
and the third one by providing a stable key (the selected person's id).
...
The effect won't re-run, even though we click on a different person, and thus the draft state stays the same.
