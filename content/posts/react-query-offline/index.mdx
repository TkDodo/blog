...
Now those things wouldn't work if React Query would decide to _not_ fire the request because you have no network connection, like the default `online` mode does. To intercept a fetch request, it must happen :) So if you have this additional cache layer, make sure to use `newtorkMode: 'offlineFirst'`.
