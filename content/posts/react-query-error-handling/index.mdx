...
This will now only show an error toast once for each query, which exactly what we want 🥳. It is also likely the best place to put any sort of error tracking or monitoring that you want to perform, because it's guaranteed to run only once per query and _cannot_ be overwritten like e.g. the defaultOptions.
