...
To solve the second issue, we can use error boundaries
...
One thing that Error Boundaries cannot do is catch asynchronous errors, because they do not occur during rendering. So to make Error Boundaries work in React Query, the library simply catches the error for you and re-throws it in the next render cycle so that the Error Boundary can pick it up.
...
This also works for mutations, and is quite helpful for e.g. form submissions to handle errors in the 4xx range locally (e.g. if some backend validation failed), but propagate all 5xx server errors to the Error Boundary.

### Showing Error Notifications

For some use-cases, like e.g. for _mutations_, it might be better to show error toast notifications that pop up somewhere (and disappear automatically) instead of rendering Alert banners on the screen. These are usually opened with an imperative api, like the one offered by [react-hot-toast](https://react-hot-toast.com/):
...
const useTodos = () =>
  useQuery(['todos'], fetchTodos, {
    // ⚠️ looks good, but is likely _not_ what you want
    onError: (error) =>
...
Conceptually, you can think that the onError callback functions similar to a _useEffect_. So if we expand the above example to that syntax, it will become more apparent that this will run for every consumer:
...
So we don't really want to notify all Observers that our fetch failed - we want to notify our user _once_ that the underlying fetch failed. For that, React Query has global callbacks on a different level:
...
This will now only show an error toast once for each query, which exactly what we want.
...
- the onError callback (on the QueryCache)
