...
But using them for parameters to a function can be an indication of a bad software design for various reasons:

### Violation of single responsibility principle

A function should do one thing, and one thing only. Passing a "flag" to it makes a function do two things at once, for example:

```ts:title=single-responsibility
// 🚨 booleans as a function parameter
function processPayment(user: User, amount: number, sendEmail: boolean) {
