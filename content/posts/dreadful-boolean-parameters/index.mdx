...
There seem to be some cases where we want to send the report as email directlyv after creating it, and some cases where we don't. But why entangle this in the _createReport_ function? That function should only create the report, and nothing else. The caller can decide what they want to do with it.

### They are confusing

Flags can be confusing, especially in languages where you don't have named parameters. Take for example this signature from the Kotlin standard library of [equals](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/equals.html):
...
This is rather rudimentary number formatting function, but apart from that, it doesn't look too bad. Frankly, the first "flag" you add to a function looks _very innocent_.
...
I can recommend them all, but for starters, resist the urge of adding the next boolean parameter to your codebase
