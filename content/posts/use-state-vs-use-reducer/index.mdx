...
<UsToc id="use-state-vs-use-reducer" />
...
const useForm = <State extends Record<string, unknown>>(
  ...
  const update = <Key extends keyof State>(
    name: Key,
    value: State[Key],
...
&#x200B;<Emph>Update</Emph>: React 18 provides the
[useSyncExternalStore](https://reactjs.org/docs/hooks-reference.html#usesyncexternalstore)
hook, so libraries are moving away from _forceUpdate_.
...
function App() {
  ...
  return (
    <div>
      Count: {count}
      <button
        onClick={() => dispatch({ type: 'set', value: count + 1 })}
      >
        Increment
      </button>
      <button
