...
<RqToc id="react-query-render-optimizations" />
...
<Translations
  translations={[
    ...
    {
      language: "日本語",
      url: "https://zenn.dev/matazou/articles/0645957b2b9168",
    },
  ]}
/>
...
```ts:title=optimized-with-notifyOnChangeProps {1,6,9}
...
if (someCondition()) {
    // 🟡 we will track the data field if someCondition
    // was true in any previous render cycle
...
Starting with v4, tracked queries are turned on per default in React
Query, and you can opt out of the feature with `notifyOnChangeProps: 'all'`.
