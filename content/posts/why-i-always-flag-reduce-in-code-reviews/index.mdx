...
Everyone understands what is going on when we read `values.join(';')`. Compare that to the above implementation - I think we can agree that the simplicity is preferred.
...
Yes, there is no easy native way to do that, and not even popular util libraries like _lodash_ have a good way of achieving this
([keyBy](https://lodash.com/docs/4.17.15#keyBy) is okay, but doesn't transform values).

In one project I am working, we frequently found the for such transformations, so we made our own util for it.
...
If your toObject does _one_ iteration with a reduce or _two_ iterations with a _map_ followed by _Object.fromEntries_ is irrelevant,
...
Mutation is not the root of all evil, it has _not_ have to be avoided at all costs.
If the scope is small, the intent is clear - mutate away 🚀
